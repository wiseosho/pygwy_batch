...
for key in DataFields.keys():
	...
	first_filename = Cont_Dest.get_string_by_name("/filename")
	if (title == 'Amplitude') : Cont_Dest.remove_by_prefix('/'+key.split('/')[1]+'/')
	Cont_Dest.set_string_by_name(key+'/title', title+'.'+str(first_filename))
...
for key in DataFields.keys():
	...
	ind_med = [i for i,v in enumerate(data) if data[i] == median]
	...
	Hist_pct = []
	Hist_pct.append( (float(index))/512 for index in range(ind_med, 0, -1)  if (data[index] >= ratio[0] and data[index-1] <= ratio[0]) )
	Hist_pct.append( (float(index))/512 for index in range(ind_med, 512, 1)  if (data[index] <= ratio[0] and data[index+1] >= ratio[0]) )
